...
import { OTHER_USERS, POSTS, CLUBS } from '../data';
...
const UserProfile = () => {
  ...
  const user = OTHER_USERS.find(u => u.id === id);
  const userPosts = POSTS.filter(p => (p as any).authorId === id);
  const userClubs = CLUBS.filter(c => (user as any).joinedClubs?.includes(c.id));

  if (!user) return <div className="p-10 text-white font-black text-center text-2xl">Kullanıcı bulunamadı</div>;
  ...
  return (
    <div className="pb-10">
      ...
      <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b-4 border-black p-4 z-20 shadow-md flex justify-between items-center">
         ...
         <span className="text-xs bg-black text-white px-2 py-1 rounded font-bold">{userPosts.length} Gönderi</span>
      </div>
      ...
      <div className="p-4 space-y-6 max-w-3xl mx-auto">
        ...
        <div className="bg-white border-4 border-black rounded-2xl overflow-hidden shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <div className="h-40 bg-gradient-to-r from-pink-400 to-purple-500 border-b-4 border-black relative"></div>
            <div className="px-6 pb-6 relative">
                <div className="flex justify-between items-start">
                    <div className="-mt-16 mb-3">
                        <img src={user.avatar} className="w-32 h-32 rounded-full border-4 border-white outline-4 outline-black bg-white" alt="Avatar" />
                    </div>
                ...
                <div className="flex flex-col items-start gap-1">
                    <h1 className="text-3xl font-black leading-none">{user.name}</h1>
                    {(user as any).title && (
                ...
                <p className="text-gray-500 font-bold mb-4 mt-1">{user.handle}</p>
                <p className="text-lg mb-6 font-medium leading-tight">{user.bio}</p>
                <div className="flex flex-wrap gap-4 text-gray-500 font-bold text-sm mb-6 border-b-4 border-gray-100 pb-4">
                    ...
                    <span className="flex items-center gap-1"><HiLink /> beykent.edu.tr</span>
                    <span className="flex items-center gap-1"><HiCalendarDays /> {user.joined} tarihinde katıldı</span>
                </div>
                <div className="flex gap-6 font-bold">
                    <span><span className="text-black text-2xl">{user.following}</span> Takip edilen</span>
                    <span><span className="text-black text-2xl">{user.followers}</span> Takipçi</span>
                </div>
            </div>
            <div className="flex border-t-4 border-black">
                ...
                <div onClick={() => setActiveTab('clubs')} className={`flex-1 text-center py-4 font-black cursor-pointer hover:bg-gray-50 ${activeTab === 'clubs' ? 'bg-[#fbca1f] text-black border-l-4 border-black' : 'text-gray-500'}`}>KULÜPLERİ</div>
            </div>
        </div>
           ...
           {activeTab === 'posts' ? (
               <div className="space-y-4">
                  {userPosts.length > 0 ? (
                          ...
                          <article key={post.id} className="bg-white border-4 border-black rounded-2xl p-4 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all cursor-pointer">
                            <div className="flex gap-4">
                                <div className="shrink-0">
                                    <img src={user.avatar} alt={user.name} className="w-14 h-14 rounded-full border-2 border-black" />
                                </div>
                                <div className="flex-1">
                                    <div className="flex flex-wrap items-center gap-x-2 mb-1">
                                        <span className="font-black text-lg text-black">{user.name}</span>
                                        <span className="text-gray-500 font-bold text-sm">{user.handle}</span>
                      ...
                      <div className="bg-white border-4 border-black rounded-2xl p-8 text-center text-gray-500 font-bold shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">Bu kullanıcının henüz gönderisi yok.</div>
                  )}
               </div>
           ) : (
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {userClubs.length > 0 ? (
                    ...
                    <div className="col-span-2 bg-white border-4 border-black rounded-2xl p-8 text-center text-gray-500 font-bold shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">Görünen bir kulüp üyeliği yok.</div>
                  )}
               </div>
           )}
        </div>
