...
import { HiHome, HiHashtag, HiBell, HiUser, HiMagnifyingGlass, HiArrowRightOnRectangle, HiBriefcase, HiLightBulb, HiCog6Tooth } from 'react-icons/hi2';
...
const Layout = () => {
  ...
  const [darkMode, setDarkMode] = useState(true); // Varsayılan: Koyu Mod (Ampul Sönük)
  ...
  return (
    ...
    <div className={`min-h-screen font-['Oswald'] overflow-hidden relative transition-colors duration-500 ${darkMode ? 'bg-[#111111]' : 'bg-[#f4f1ea]'}`}>
      ...
      <div className="container mx-auto max-w-[1600px] flex gap-0 md:gap-8 px-0 md:px-4 h-screen relative z-10">
        ...
        <div className="hidden lg:block w-80 h-full py-6 pl-2 shrink-0 space-y-6 overflow-y-auto no-scrollbar">
          ...
          <div className="bg-white border-4 border-black rounded-2xl p-4 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <h3 className="text-xl font-black mb-4">KULÜPLERİ KEŞFET</h3>
            <div className="space-y-2">
              {CLUBS.map(club => (
                <Link to={`/home/club/${club.id}`} key={club.id} className="flex items-center gap-3 p-2 -mx-2 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer group">
                  ...
                  <div className="flex-1 overflow-hidden">
                    <p className="font-bold truncate text-black group-hover:underline decoration-2">{club.name}</p>
                    <p className="text-gray-500 text-xs truncate">{club.handle}</p>
                  </div>
